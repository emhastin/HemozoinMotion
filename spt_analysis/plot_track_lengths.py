...
track_length_systems = sys.argv[1:]
...
axa.set_xlabel(r'Track length')
axa.set_ylabel(r'Number of tracks')
...
# length2time = lambda x: (duration[2]-duration[1])*x
# time2length = lambda x: x/(duration[2]-duration[1])

# secax = axa.secondary_xaxis('top', functions=(length2time, time2length))
# secax.set_xlabel(r'Track duration $(\si{\second})$')

colors = ['black', 'red', 'blue', 'green', 'orange']

for j, track_length_system in enumerate(track_length_systems):

	track_length_filename = 'data/track_lengths_{}.dat'.format(track_length_system)

	track_lengths, duration, prob = np.transpose( np.genfromtxt(track_length_filename, skip_header=1) )

	axa.bar(track_lengths, prob, width=track_lengths[1]-track_lengths[0], color = colors[j], label = '{}'.format(track_length_system), fill=False)
