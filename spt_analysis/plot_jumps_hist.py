...
axa.set_ylabel(r'$P (\vert \Delta r \vert)$')
...
for j, jump_system in enumerate(jump_systems):
	...
	jump_filename = 'data/jumps_hist_{}_lag_{}.dat'.format(jump_system, chosen_lag)

	jumps, prob = np.transpose( np.genfromtxt(jump_filename, skip_header=1) )

	axa.bar(jumps, prob, width=jumps[1]-jumps[0], color = colors[j], label = '{}'.format(jump_system), fill=False)
