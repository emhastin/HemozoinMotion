...
Ns = np.zeros(len(times),dtype=int)
with open(data_filename, 'r') as input_file:
	for line in input_file:
		if line[0] == '#': continue
		line_split = line.split(' ')
...
for j, s in enumerate(sd_list):
	...
	if N == 0:
		...
		dtamsd[j] = np.nan
		continue
	bootstrap_draws = np.random.choice(s, (N,bootstrap_size))
	bootstrap_means = np.mean(bootstrap_draws, axis=0)
	tamsd[j] = np.mean(bootstrap_means)
	dtamsd[j] = np.std(bootstrap_means)
	Ns[j] = N

# here I store the data in separate files
# I always store separately, and the plot separately with other script
np.savetxt('data/tamsd_tdiv_errorbars_{}.dat'.format(analyzed_system), np.transpose([times, tamsd, dtamsd, Ns]), header='t TAMSD dTAMSD N')
